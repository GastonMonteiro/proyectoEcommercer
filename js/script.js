...
botonAgregar.addEventListener("click",(e) => {
  ...
  agregados.forEach((producto) => {
    ...
    tableRow.innerHTML = `
      ...
      <button id="${producto.nombre}" class="btn btnBorrar border border-3 bgLila m-1" type="button" id="button-addon2">Eliminar</button>
    `
    tbody.appendChild(tableRow)
    const productoPrecioModificar = producto.precio
    const productoStockModificar = producto.stock
    const productoDescripcionModificar = producto.descripcion

    document.getElementById('modalPrecio').value = "$ " + productoPrecioModificar;
    document.getElementById('modaltStock').value = productoStockModificar;
    document.getElementById('modalDescripcion').value = productoDescripcionModificar;
  ...
  botonesBorrar.forEach((boton) => {
    boton.addEventListener("click", (e) => {
      let respuesta = confirm("¿Seguro quiere borrar el articulo?");
    if (respuesta) {
